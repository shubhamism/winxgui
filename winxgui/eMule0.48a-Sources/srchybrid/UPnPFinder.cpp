...
void CUPnPFinder::StopUPnPService(){
...
void CUPnPFinder::StartDiscovery(uint16 nTCPPort, uint16 nUDPPort, bool bSecondTry)
{
...
HRESULT CUPnPFinder::SaveServices(EnumUnknownPtr pEU, const LONG nTotalItems)
{
	HRESULT hr = S_OK;
...
void CUPnPFinder::CreatePortMappings(ServicePointer pService)
{
	...
	Log(GetResString(IDS_UPNPSUCCESS), m_nTCPPort, m_nUDPPort);
	SendResultMessage();
...
void CUPnPFinder::SetMessageOnResult(HWND hWindow, UINT nMessageID){
	m_hResultMessageWindow = hWindow;
	m_nResultMessageID = nMessageID;
}

void CUPnPFinder::SendResultMessage(){
	if (m_hResultMessageWindow != 0 && m_nResultMessageID != 0){
		PostMessage(m_hResultMessageWindow, m_nResultMessageID, (WPARAM)(m_bUPnPPortsForwarded == TRIS_TRUE ? UPNP_OK : UPNP_FAILED), 0);
		m_nResultMessageID = 0;
		m_hResultMessageWindow = 0;
	}
}
			...
			LPCWSTR pszStateVarName, VARIANT varValue)
{
	...
	if ( !m_instance.IsAsyncFindRunning() )
	{
		...
		{
			theApp.m_pUPnPFinder->m_bUPnPDeviceConnected = strValue.CompareNoCase( L"Disconnected" ) == 0
					? TRIS_FALSE
...
CString translateUPnPResult(HRESULT hr)
{
	...
	if ( hr >= UPNP_E_ACTION_SPECIFIC_BASE && hr <= UPNP_E_ACTION_SPECIFIC_MAX )
		return CString("Action Specific Error");
