...
// $Id: mfcievw.cpp,v 1.8 2006/04/26 08:31:59 wangyingqi Exp $
//
...
void CMfcieView::OnInitialUpdate()
{
	...
	//m_searcher.Init(); //@@modify - init
	WCHAR indexPath[MAX_PATH];
	GetSystemIniPath(indexPath, INDEXING_DIR);
	m_searcher.init(indexPath, indexPath, indexPath);

	WCHAR inputPath[MAX_PATH];
	GetSystemIniPath(inputPath, INPUT_DIR);
	m_searcher.crawl(inputPath);
...
// $Log: mfcievw.cpp,v $
// Revision 1.8  2006/04/26 08:31:59  wangyingqi
// ifu一些编不过的文件整理
//
// Revision 1.7  2006/02/18 08:58:01  liucong
// no message
//
// Revision 1.6  2006/02/17 06:48:04  liucong
// 稍微调整了一下searcher相关的格式
//
// Revision 1.5  2006/01/09 08:04:01  xushiwei
// 支持ks_string, ks_wstring。
//
